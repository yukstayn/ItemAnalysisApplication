...
upload_file = st.file_uploader( label = 'Choose a CSV File', type = 'csv')
...
if upload_file is not None:
	...
	df = pd.DataFrame(pd.read_csv(upload_file)).astype(int)
	df.index = df.index + 1
	st.write(df)
	...
	slider = st.slider('Choose the number of Upper and Lower Group', min_value = int(df.index.min()), max_value = int(df.index.max()))
	...
	fig = go.Figure(data=[
	    go.Bar(name='Diff. Index', x=new_df.columns, y=diff_index),
	    go.Bar(name='Disc. Index', x=new_df.columns, y=disc_index)
	])
